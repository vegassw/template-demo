  ...
  if (petCart.length === 0 && step !== 'success') {
    return (
      <Page className="max-w-3xl mx-auto px-6 py-32 text-center">
        <h1 className="font-heading text-3xl text-[#2E3A35] mb-4">No hay productos para pagar</h1>
        <Link to="/servicios/tienda" className="text-[#7BAE8F] hover:underline">Ir a la tienda</Link>
      </Page>
    );
  }
    ...
    return (
      <Page className="min-h-[70vh] flex items-center justify-center px-6">
        <div className="text-center max-w-sm" data-testid="pet-checkout-processing">
          ...
          <p className="font-heading text-2xl text-[#2E3A35] mb-2">Procesando tu pago…</p>
    ...
    return (
      <Page className="min-h-[70vh] flex items-center justify-center px-6 py-16">
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center max-w-md" data-testid="pet-checkout-success">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', delay: 0.1 }} className="w-20 h-20 rounded-full bg-[#7BAE8F]/15 flex items-center justify-center mx-auto mb-8">
            <CheckCircle2 size={48} className="text-[#7BAE8F]" />
          </motion.div>
          <h1 className="font-heading text-3xl md:text-4xl text-[#2E3A35] mb-3">¡Pago aprobado!</h1>
          <p className="text-[#5C6660] mb-6">Tu compra fue procesada con éxito. Recibirás un correo con los detalles de tu pedido.</p>
          <div className="bg-white border border-[#E8E0D5] rounded-2xl p-6 mb-8 text-left">
            ...
            <div className="flex justify-between text-sm mb-2"><span className="text-[#5C6660]">Total pagado</span><span className="font-semibold text-[#7BAE8F]">{formatCLP(total)}</span></div>
          ...
          <Link to="/servicios/tienda" className="inline-flex items-center gap-2 bg-[#2E3A35] text-white px-8 py-4 rounded-full text-sm font-semibold hover:bg-[#1f2823] transition-colors" data-testid="pet-back-shop-btn">
            Volver a la tienda
          </Link>
        </motion.div>
      </Page>
  ...
  return (
    <Page className="max-w-6xl mx-auto px-6 md:px-10 py-12">
      <div data-testid="pet-checkout-page">
        <Link to="/servicios/carrito" className="inline-flex items-center gap-2 text-sm text-[#5C6660] hover:text-[#7BAE8F] mb-8"><ArrowLeft size={16} /> Volver al carrito</Link>
        <h1 className="font-heading text-4xl md:text-5xl text-[#2E3A35] mb-10">Finalizar compra</h1>

        <form onSubmit={handleSubmit} className="grid lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-10">
            <section>
              <h2 className="font-heading text-xl text-[#2E3A35] mb-5">Datos de contacto y envío</h2>
              <div className="grid sm:grid-cols-2 gap-4">
                <input required value={form.nombre} onChange={set('nombre')} placeholder="Nombre completo" className="border border-[#E8E0D5] rounded-xl px-4 py-3 focus:border-[#7BAE8F] focus:outline-none sm:col-span-2" data-testid="pet-input-nombre" />
                <input required type="email" value={form.email} onChange={set('email')} placeholder="Correo electrónico" className="border border-[#E8E0D5] rounded-xl px-4 py-3 focus:border-[#7BAE8F] focus:outline-none" data-testid="pet-input-email" />
                <input required value={form.telefono} onChange={set('telefono')} placeholder="Teléfono" className="border border-[#E8E0D5] rounded-xl px-4 py-3 focus:border-[#7BAE8F] focus:outline-none" data-testid="pet-input-telefono" />
                <input required value={form.direccion} onChange={set('direccion')} placeholder="Dirección" className="border border-[#E8E0D5] rounded-xl px-4 py-3 focus:border-[#7BAE8F] focus:outline-none sm:col-span-2" data-testid="pet-input-direccion" />
                <input required value={form.ciudad} onChange={set('ciudad')} placeholder="Ciudad" className="border border-[#E8E0D5] rounded-xl px-4 py-3 focus:border-[#7BAE8F] focus:outline-none sm:col-span-2" data-testid="pet-input-ciudad" />
              </div>
            </section>

            <section>
              <h2 className="font-heading text-xl text-[#2E3A35] mb-5">Método de pago</h2>
              <div className="border-2 rounded-2xl overflow-hidden" style={{ borderColor: MP_BLUE }}>
                ...
                <div className="p-5 space-y-3">
                  <label className="flex items-center gap-3 p-3 border border-[#E8E0D5] rounded-xl cursor-pointer hover:border-[#7BAE8F]">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white border border-[#E8E0D5] rounded-2xl p-8 sticky top-28">
              <h2 className="font-heading text-xl text-[#2E3A35] mb-6 flex items-center gap-2"><PawPrint size={20} className="text-[#7BAE8F]" /> Tu pedido</h2>
              ...
              <div className="space-y-2 text-sm border-t border-[#E8E0D5] pt-4 mb-6">
                ...
                <div className="flex justify-between pt-2 border-t border-[#E8E0D5] text-base"><span className="font-semibold">Total</span><span className="font-heading text-xl text-[#7BAE8F]">{formatCLP(total)}</span></div>
              </div>
